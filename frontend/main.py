...
if not is_authenticated():
    ...
    st.markdown(f"<p style='text-align: center;'><img src='{logo}' width=200></p>", unsafe_allow_html=True)
    st.markdown("<h1 style='text-align: center;'>H&M Analytics</h1>", unsafe_allow_html=True)
    ...
    if st.session_state.account_created:
        st.sidebar.info("Your account has been created.")
        st.session_state.account_created = False
...
############################################################################################################################
    ...
    st.sidebar.image('https://upload.wikimedia.org/wikipedia/commons/5/53/H%26M-Logo.svg', width=130)

    st.write('<h1 style="text-align: center;">H&M Analytics</h1>', unsafe_allow_html=True)
    st.markdown('<h6 style="text-align: center;">This dashboard provides an overview of different article, customer, and transaction KPIs for our company, H&M.</h6>', unsafe_allow_html=True)
    ...
    page_options = [
        f"{emoji_customers} Customers",
        f"{emoji_articles} Articles",
        f"{emoji_transactions} Transactions",
    ]
...
############################################################################################################################
    # CUSTOMERS' PAGE
############################################################################################################################
    if page == f"{emoji_customers} Customers":
        ...
        # KPI:
        def custom_metric1(label, value):
            return f"""
            <div style="display: flex; flex-direction: column; align-items: flex-start;">
                <div style="font-weight: 500; font-size: 0.9rem; color: white;">{label}</div>
                <div style="font-weight: 700; font-size: 2.4rem; color: #D62727;">{value}</div>
            </div>
            """
        ...
        # KPI: Number of different customers, average age, maximum age, minimum age
        kpi1, kpi2, kpi3, kpi4 = st.columns(4)
        
        kpi1.markdown(custom_metric1("Total Customers", num_customers), unsafe_allow_html=True)
        kpi2.markdown(custom_metric1("Average Age", round(customers_df["age"].mean(), 1)), unsafe_allow_html=True)
        kpi4.markdown(custom_metric1("Maximum Age", customers_df["age"].max()), unsafe_allow_html=True)
        kpi3.markdown(custom_metric1("Minimum Age", customers_df["age"].min()), unsafe_allow_html=True)
        ...
        fig1 = px.histogram(customer_df, x="age", nbins=20, color_discrete_sequence=[hm_red], opacity=1.0)
        fig1.update_layout(title=dict(text="<b>Customer Age Distribution</b>", font=dict(size=24, color=hm_black), x=0, y=0.95),
                    xaxis_title=dict(text="<b>Age</b>", font=dict(size=16, color=hm_black)),
                    yaxis_title=dict(text="<b>Customers</b>", font=dict(size=16, color=hm_black)),
                    font=dict(color=hm_black),
                    plot_bgcolor='rgba(0,0,0,0)',
                    margin=dict(l=0))
        fig1.update_xaxes(showgrid=False, zeroline=False, tickfont=dict(size=14, color=hm_black))
        fig1.update_yaxes(showgrid=False, zeroline=False, tickfont=dict(size=14, color=hm_black))
        st.plotly_chart(fig1)
        
    
        def custom_metric2(label, value):
            return f"""
            <div style="display: flex; flex-direction: column; align-items: flex-start;">
                <div style="font-weight: 500; font-size: 0.9rem; color: white;">{label}</div>
                <div style="font-weight: 700; font-size: 2.4rem; color: #D62727;">{value}</div>
        ...
        # Age group KPIs
        st.markdown("### Important Customer Groups")
        kpi5, kpi6, kpi7 = st.columns(3)
        kpi8, kpi9, kpi10 = st.columns(3)
        
        perc_20to30 = ((customer_df["age"] >= 20) & (customer_df["age"] < 30)).mean() * 100
        perc_30to40 = ((customer_df["age"] >= 30) & (customer_df["age"] < 40)).mean() * 100
        perc_40to50 = ((customer_df["age"] >= 40) & (customer_df["age"] < 50)).mean() * 100
        perc_50to60 = ((customer_df["age"] >= 50) & (customer_df["age"] < 60)).mean() * 100
        perc_60to70 = ((customer_df["age"] >= 60) & (customer_df["age"] < 70)).mean() * 100
        perc_above70 = (customer_df["age"] >= 70).mean() * 100

        kpi5.markdown(custom_metric2("20 - 30 yrs", str(round(perc_20to30, 1)) + '%'), unsafe_allow_html=True)
        kpi6.markdown(custom_metric2("30 - 40 yrs", str(round(perc_30to40, 1)) + '%'), unsafe_allow_html=True)
        kpi7.markdown(custom_metric2("40 - 50 yrs", str(round(perc_40to50, 1)) + '%'), unsafe_allow_html=True)
        kpi8.markdown(custom_metric2("50 - 60 yrs", str(round(perc_50to60, 1)) + '%'), unsafe_allow_html=True)
        kpi9.markdown(custom_metric2("60 - 70 yrs", str(round(perc_60to70, 1)) + '%'), unsafe_allow_html=True)
        kpi10.markdown(custom_metric2("Above 70 yrs", str(round(perc_above70, 1)) + '%'), unsafe_allow_html=True)
        ...
        fig2 = go.Figure()
        fig2.add_trace(go.Bar(x=news_labels, y=news_counts, marker=dict(color=['red', 'green', 'blue']), opacity=0.7))
        fig2.update_layout(title=dict(text="<b>Fashion News Frequency</b>", font=dict(size=24, color=hm_black), x=0, y=0.95),
                    xaxis_title=dict(text="<b>Frequency</b>", font=dict(size=16, color=hm_black)),
                    yaxis_title=dict(text="<b>Customers</b>", font=dict(size=16, color=hm_black)),
                    font=dict(color=hm_black),
                    plot_bgcolor='rgba(0,0,0,0)',
                    margin=dict(l=0))
        fig2.update_xaxes(showgrid=False, zeroline=False, tickfont=dict(size=14, color=hm_black))
        fig2.update_yaxes(showgrid=False, zeroline=False, tickfont=dict(size=14, color=hm_black))
        st.plotly_chart(fig2)
        
        st.markdown("<h3 style='font-size: 24px;'>All Customers</h3>", unsafe_allow_html=True)
...
############################################################################################################################
    # ARTICLES' PAGE
#############################################################################################################################
    elif page == f"{emoji_articles} Articles":
        ...
        kpi1.markdown(custom_metric1("Total Articles", len(filtered_df)), unsafe_allow_html=True)
        kpi2.markdown(custom_metric1("Number of Unique Products", len(filtered_df['product_type_name'].unique())), unsafe_allow_html=True)
        ...
        with sns.color_palette(["#373F51", "#F9A03F", "#4E8A8B", "#F13C20", "#F8E9A1", "#6A8A82"]):
            ...
            fig2.layout.legend.title = "<b>Graphical Appearance</b>"
            fig2.layout.legend.x = 1
            ...
            fig4.layout.legend.title = "<b>Graphical Appearance</b>"
            ...
            for fig in [fig2, fig4]:
                ...
                fig.update_layout(
                    ...
                    width=800,
                    height=800,
                    margin=dict(l=10, r=50, b=100, t=100, pad=4)
                )
        ...
        st.markdown("<h3 style='font-size: 24px;'>All Articles</h3>", unsafe_allow_html=True)
...
############################################################################################################################
    # TRANSACTIONS' PAGE
#############################################################################################################################
    elif page == f"{emoji_transactions} Transactions":
        ...
        # Convert the min and max 'price' values to Python float
        min_price = float(df['price'].min())
        max_price = float(df['price'].max())

        # Create the Streamlit slider using the converted min and max values
        ...
        def custom_metric3(label, value):
            return f"""
            <div style="display: flex; flex-direction: column; align-items: flex-start;">
                <div style="font-weight: 500; font-size: 0.9rem; color: white;">{label}</div>
                <div style="font-weight: 700; font-size: 2.4rem; color: #D62727;">{value}</div>
        ...
        kpi_space1, kpi4, kpi5, kpi_space2 = st.columns([1, 2, 2, 1])

        st.write("")
        st.write("")
        
        kpi4.markdown(custom_metric3("Total Sales Revenue", round(price_sum, 2)), unsafe_allow_html=True)
        kpi5.markdown(custom_metric3("Number of Transactions", transaction_count), unsafe_allow_html=True)
        ...
        fig1.update_traces(textposition='inside', textinfo='percent+label', showlegend=False)  # Remove legend
        fig1.update_layout(
            width=400,  # Update width
            height=400,  # Update height
        )
        fig1.layout.title = {
            'text': "<b>Revenue per Sales Channel</b>",
            'x': 0.5,
            'xanchor': 'center',
            'font': {'size': 24}
        }
        ...
        fig2.update_traces(textposition='inside', textinfo='percent+label', showlegend=False)  # Remove legend
        fig2.update_layout(
            width=400,  # Update width
            height=400,  # Update height
        )
        fig2.layout.title = {
            'text': "<b>Volume per Sales Channel</b>",
            'x': 0.5,
            'xanchor': 'center',
            'font': {'size': 24}
        }
        ...
        st.markdown("<h3 style='font-size: 24px;'>All Transactions</h3>", unsafe_allow_html=True)
